node{
    ...
    stage('git code checkout'){
        try{
            ...
            git 'https://github.com/Dhananjay6736/star-agile-insurance-project.git'
        }
        catch(Exception e){
    ...
    stage('Containerize the application'){
        ...
        sh "${dockerCMD} build -t dhananjay6736/insure-me:${tagName} ."
    }
    
    stage('Pushing it ot the DockerHub'){
        echo 'Pushing the docker image to DockerHub'
        withCredentials([string(credentialsId: 'dock-password', variable: 'dockerHubPassword')]) {
        sh "${dockerCMD} login -u dhananjay6736 -p ${dockerHubPassword}"
        sh "${dockerCMD} push dhananjay6736/insure-me:${tagName}"
